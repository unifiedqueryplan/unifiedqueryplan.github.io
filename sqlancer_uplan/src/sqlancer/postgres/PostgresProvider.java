...
public class PostgresProvider extends SQLProviderAdapter<PostgresGlobalState, PostgresOptions> {
    ...
    public String getQueryPlan(String selectStr, PostgresGlobalState globalState) throws Exception {
        ...
        SQLQueryAdapter q = new SQLQueryAdapter("EXPLAIN (FORMAT JSON, VERBOSE TRUE, SUMMARY TRUE) " + selectStr, errors);
        ...
        if (!queryPlan.isEmpty()) {
            return DBMSCommon.parseQueryPlan(queryPlan, "postgresql", "qpg", globalState.getOptions().getPathUplan());
        }
        else {
            return "";
        }
